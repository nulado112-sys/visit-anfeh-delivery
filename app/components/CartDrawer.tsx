          ...
          {items.length === 0 && orderNumber ? (
            <div className="flex h-full flex-col items-center justify-center gap-4 px-6 text-center">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-slate-400">Order <span className="font-bold text-[#1AABBD]">{orderNumber}</span> is being processed</p>
              </div>
              {waUrl && (
                <a
                  href={waUrl}
                  target="_blank"
                  rel="noreferrer"
                  className="flex w-full items-center justify-center gap-2 rounded-2xl bg-[#25D366] py-3 text-sm font-bold text-white"
                >
              ...
              <a
                ...
                onClick={closeCart}
                className="w-full rounded-2xl border border-[#1AABBD]/40 py-3 text-sm font-bold text-[#1AABBD]"
              >
                Track Your Order →
              </a>
              <button onClick={() => { setOrderNumber(null); setWaUrl(null); }} className="text-xs text-slate-600 hover:text-slate-400">
                Close
              </button>
            </div>
          ) : items.length === 0 ? (
